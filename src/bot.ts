...
export class Bot {
    ...
    //setBotName(botName: string): 这个方法允许你更改机器人的名称。
    setBotName(botName: string) {
        this.botName = botName;
    }
    //onMessage(message: Message): 这是主要的事件处理函数，当接收到新的聊天消息时，它会被调用。
    //函数内部实现了多种判断和操作：首先，获取消息的时间、内容、发送者等信息，以及消息所在的聊天室的主题。
    //然后，检查消息是否在聊天室内发送，若不在则返回。再判断消息是否为无意义的消息或包含违禁词。
    //若是无意义的消息则不进行后续操作，若包含违禁词，则向聊天室发送提示信息，并不进行后续操作。
    //如果消息内容为'/help'，则返回一个帮助信息，包含一些命令和参数的解释和用法。通过前缀检查消息是否是指令（开始于'/imagine '或'/up '）。
    //如果不是，不进行后续操作。若是指令，根据指令的类型调用submitTask函数提交任务，并处理可能出现的错误。
    async onMessage(message: Message) {
        ...
        if (rawText == '/help') {
            const result = "欢迎使用 Starry-Gaze MJ机器人\n" +
                "------------------------------\n"
                + "🎨 生成图片命令\n"
                + "输入: /imagine prompt\n"
                + "prompt 即你向mj提的绘画需求\n"
